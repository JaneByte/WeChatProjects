...
-- Update existing SKUs to standard type
UPDATE `goods_sku` SET `spec_type` = 'standard', `spec_value` = '标准装' WHERE `spec_type` IS NULL;

-- Insert multi-spec SKUs for product 13 (红富士苹果)
...
-- Insert sku_component for platter SKU 10003
...
INSERT INTO `sku_component`(`sku_id`, `component_goods_id`, `component_name`, `component_weight_g`, `quantity`, `component_price`, `sort`)
SELECT 10003, 49, '黄瓜', 300, 1, 6.20, 2
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `sku_component` WHERE `sku_id` = 10003 AND `component_goods_id` = 49 AND `sort` = 2);

INSERT INTO `sku_component`(`sku_id`, `component_goods_id`, `component_name`, `component_weight_g`, `quantity`, `component_price`, `sort`)
SELECT 10003, 77, '西红柿', 300, 1, 7.80, 3
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `sku_component` WHERE `sku_id` = 10003 AND `component_goods_id` = 77 AND `sort` = 3);
