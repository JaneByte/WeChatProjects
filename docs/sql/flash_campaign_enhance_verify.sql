SELECT COUNT(*) AS flash_total FROM `goods` WHERE `status` = 1 AND `is_flash` = 1;
...
SELECT `id`,`name`,`price`,`flash_price`,`flash_stock`,`stock`,`flash_start_time`,`flash_end_time`
FROM `goods`
WHERE `status` = 1 AND `is_flash` = 1
ORDER BY `flash_start_time` ASC, `id` ASC
LIMIT 30;