...
public interface TrackMapper {
    ...
    @Select("SELECT * FROM goods WHERE status = 1 AND show_in_home = 1 AND stock > 0 AND (name LIKE CONCAT('%', #{keyword}, '%') OR keywords LIKE CONCAT('%', #{keyword}, '%')) ORDER BY sales_volume DESC, home_sort ASC, create_time DESC LIMIT #{limit}")
